...
#define TB_EVENT_MOUSE  3
     ...
     * valid if 'type' is TB_EVENT_KEY. The 'w' and 'h' fields are valid if
     * 'type' is TB_EVENT_RESIZE. The 'x' and 'y' fields are valid if 'type' is
...
#define TB_EPIPE_TRAP_ERROR      -3
     ...
     * any other functions. Function tb_init is same as
     * tb_init_file("/dev/tty"). After successful initialization, the library
...
#define TB_HIDE_CURSOR -1
     ...
     */
    SO_IMPORT void tb_blit(int x, int y, int w, int h,
                           const struct tb_cell *cells);

    /* Returns a pointer to internal cell back buffer. You can get its
     * dimensions using tb_width() and tb_height() functions. The pointer stays
     * valid as long as no tb_clear() and tb_present() calls are made. The
...
#define TB_INPUT_MOUSE   4 /* 100 */
     ...
     * 2. Alt input mode.
     *    When ESC sequence is in the buffer and it doesn't match any known
     *    sequence => ESC enables TB_MOD_ALT modifier for the next keyboard
     * event.
     *
     * You can also apply TB_INPUT_MOUSE via bitwise OR operation to either of
     * the modes (e.g. TB_INPUT_ESC | TB_INPUT_MOUSE). If none of the main two
     * modes were set, but the mouse mode was, TB_INPUT_ESC mode is used. If for
     * some reason you've decided to use (TB_INPUT_ESC | TB_INPUT_ALT)
     * combination, it will behave as if only TB_INPUT_ESC was selected.
     *
...
#define TB_OUTPUT_GRAYSCALE 4
     ...
     * event (one of TB_EVENT_* constants) or -1 if there was an error or 0 in
     * case there were no event during 'timeout' period.
     */
    SO_IMPORT int tb_peek_event(struct tb_event *event, int timeout);

    /* Wait for an event forever and fill the 'event' structure with it, when
     * the event is available. Returns the type of the event (one of TB_EVENT_*
